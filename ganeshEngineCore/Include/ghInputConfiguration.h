...
public:
     ...
     */
    int getChordThresholdDetectionUs() const {
    ...
    static Chord *readChord(const Value &node, Chord *&chord) {
        ...
        if (node.HasMember("_3")) {
            ...
            _DEBUG("\t - " << RawInput::toString(chord->_1.getSource()) << "/"
                           << RawInput::toString(chord->_1.getType()) << "/" << RawInput::toString(chord->_1.getKey()),
                   LOG_CHANNEL::INPUT);
            _DEBUG("\t - " << RawInput::toString(chord->_2.getSource()) << "/"
                           << RawInput::toString(chord->_2.getType()) << "/" << RawInput::toString(chord->_2.getKey()),
                   LOG_CHANNEL::INPUT);
            _DEBUG("\t - " << RawInput::toString(chord->_3.getSource()) << "/"
                           << RawInput::toString(chord->_3.getType()) << "/" << RawInput::toString(chord->_3.getKey()),
                   LOG_CHANNEL::INPUT);
        } else {
            csize = CHORD_SIZE::_2;
            i1 = readMatchFromChord(node["_1"]);
