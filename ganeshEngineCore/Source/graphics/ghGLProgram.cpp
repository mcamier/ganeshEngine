        ...
        for (GLuint i = 0; i < (GLuint) params; i++) {
            ...
            if (size > 1) {
                for (int j = 0; j < size; j++) {
                    ...
                    _DEBUG("\t\t" << i << "> type: " << GL_type_to_string(type) << " name: " << long_name
                                  << " location: " << location, LOG_CHANNEL::RENDER);
                }
            } else {
                int location = glGetAttribLocation(mInternalId, name);
                _DEBUG("\t\t" << i << "> type: " << GL_type_to_string(type) << " name: " << name << " location: "
                              << location, LOG_CHANNEL::RENDER);
            }
        }
