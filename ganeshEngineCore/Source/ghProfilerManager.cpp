...
namespace ganeshEngine {
    ...
    void ProfilerManager::update() {
        for (auto entry : mLastFrameSamplesMap) {
            ...
            if (globalEntry == mGlobalSamplesMap.end()) {
                ...
                global.mAverageDurationMs =
                        ((global.mAverageDurationMs * global.mSamplesAmount) + entry.second.mDurationMs) /
                        (global.mSamplesAmount + 1);
    ...
    void ProfilerManager::vDestroy() {
        _DEBUG("ProfilesManager details", LOG_CHANNEL::DEFAULT);
        for (auto entry : mGlobalSamplesMap) {
            _DEBUG("\t" << entry.first << " [avg : " << entry.second.mAverageDurationMs << "] [min : "
                        << entry.second.mMinDurationMs << "] [max : " << entry.second.mMaxDurationMs << "]",
                   LOG_CHANNEL::DEFAULT);
        }
    }
