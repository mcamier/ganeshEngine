...
namespace ganeshEngine {
    ...
    void InputManager::vInitialize() {
        ...
        glfwSetKeyCallback(gPlatform().getWindow(), [](GLFWwindow *window, int key, int scancde, int action, int mods) {
            ...
            switch (action) {
                case GLFW_PRESS: {
                    ...
                    _DEBUG("rawInput registered : [KEYBOARD] [" << RawInput::toString(input.type) << "]",
                           LOG_CHANNEL::INPUT);
                    break;
                }
                case GLFW_RELEASE: {
        ...
        gEvent().addListener(GH_HASH("__GH_EVENT_JOYSTICK_STATE_CHANGE"), [](Event *e) {
	        JoystickStateChangeEvent* jsce = static_cast<JoystickStateChangeEvent*>(e);
            if(jsce->getJoystickState() == GLFW_CONNECTED) {
                _DEBUG("Joystick [" << jsce->getJoystickIndex() << "] connected", LOG_CHANNEL::DEFAULT);
            } else if (jsce->getJoystickState() == GLFW_DISCONNECTED) {
                _DEBUG("Joystick [" << jsce->getJoystickIndex() << "] disconnected", LOG_CHANNEL::DEFAULT);
            }
        });
