...
void const EventManager::fireEvent(Event* event)
{
    auto pair = m_Listeners.equal_range(event->getType());
    for (multimap<EventType, EventCallback>::iterator it = pair.first;
	    it != pair.second;
...
void const EventManager::addListener(EventType eventType, EventCallback callback)
{
    m_Listeners.insert(pair<EventType, EventCallback>(eventType, callback));
}

void const EventManager::removeAllListeners(EventType eventType)
{
