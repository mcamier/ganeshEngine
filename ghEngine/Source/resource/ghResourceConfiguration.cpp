...
void ResourceConfiguration::readResources(const Value &node, ResourceConfiguration *conf) {
    for (SizeType i = 0; i < node.Size(); i++) {
        readResource(node[i], conf);
    }
}

void ResourceConfiguration::readResource(const Value &node, ResourceConfiguration *conf) {
